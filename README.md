# Overview

Plan and info about my personal ultra running, energy intake, pacing, gear, training, races and more.

## Tänndalen 100 miles

### Gear
- ullmax underställ as extra warm layer
- craft wind/rain jacket
- headlamp
- powerbank small
- charging cables
- charging adapter for watch
- salamon adv skin 12
- 4 x soft flasks

## Must be bought
- emergency blanket https://www.jula.se/catalog/fritid/friluftsliv-och-camping/talta/talttillbehor/nodfilt-025194/
- reusable cup?
- 500 ml soft flask
- shell jacket?
- u gels
- u gels caffeine
- u bars
- u salt pills
- snickers
- pbj

### Energy

**Aid stations**

| From → To | Distance (km) |
|---|---|
| Ski Lodge Tänndalen → Skottgården | 22.1 |
| Skottgården → Rastplats "Langs bekken" | 17.4 |
| Rastplats "Langs bekken" → Røros Hotell | 17.1 |
| Røros Hotell → Rastplats "Langs bekken" | 17.1 |
| Rastplats "Langs bekken" → Skottgården | 17.4 |
| Skottgården → Ski Lodge Tänndalen | 22.1 |
| Ski Lodge Tänndalen → Bygdegård Tänndalen | 6.2 |
| Bygdegård Tänndalen → Hållanparkeringen | 6.2 |
| Hållanparkeringen → Hotell Funäsdalen | 16.5 |
| Hotell Funäsdalen → Hållanparkeringen | 6.9 |
| Hållanparkeringen → Bygdegård Tänndalen | 5.7 |
| Bygdegård Tänndalen → Ski Lodge Tänndalen | 6.2 |

**Supplies form stations**
- Water
- U sport
- Chips
- Candy
- Coffee
- Cola
* Hot food will be available every 20-30 km

**bring myself**
- u gels
- u bars
- umara u salt
- snickers
- pbj sandswich

### Time / pacing
Goal time is 24 h = 8:57 min per hm

### Summary
**90 g carbs / h**
2 or 1 salt pill / h depending on weather
2 gels / h = 48 gels
    exchange some with u bars
    intake real food at aid stations or when tired of gels
bring snickers, pbj. unsure of how many

